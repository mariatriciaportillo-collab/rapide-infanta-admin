...
const newHandlePayment = `
  const handlePayment = async () => {
    ...
    if (amount > Number(sale.balance_due)) {
...
const receiptBtn = "{/* View Receipt */}\n" +
  "              {(sale.status === 'PAID' || sale.status === 'PARTIALLY PAID' || Number(sale.amount_paid) > 0) && (\n" +
  "                <Link href={`/quick-sale/${sale.id}/receipt`} className=\"bg-blue-50 text-blue-700 hover:bg-blue-100 px-4 py-2 rounded-md font-medium text-sm transition flex items-center gap-2\">\n" +
