...
def patch_new_html():
    ...
    code = re.sub(r'<div className="md:col-span-2">\s*<label className="block text-sm font-medium text-slate-700 mb-1">Part / Product Name \*\s*</label>\s*<input \s*required \s*type="text" \s*value=\{name\}\s*onChange=\{e => setName\(e.target.value\)\}\s*className="w-full border border-slate-300 rounded-md p-2 font-medium"\s*/>\s*</div>', html, code, flags=re.MULTILINE)
