...
// 1. Add Service Details section near Customer & Vehicle Info
const custVehicleRegex = /<div className="grid grid-cols-2 divide-x divide-slate-200 border-b border-slate-200">[\s\S]+?<\/div>\s*<\/div>\s*<\/div>/;
...
if (!file.includes('Service Advisor</span>')) {
  file = file.replace(custVehicleRegex, match => match + '\n' + serviceDetailsUI);
}

// 2. Add Warranty and Authorization at the bottom
// Let's find the closing div of the main content before the return ends.
const bottomRegex = /(<\/div>\s*<\/div>\s*<\/div>\s*)$/;

const legalBlocks = `
        {/* Legal & Signatures */}
        <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-8 print:mt-4 print:gap-4 page-break-inside-avoid">
          {/* Warranty Policy */}
          <div className="border border-slate-200 rounded-lg p-4 bg-white print:p-3">
          ...
          <div className="border border-slate-200 rounded-lg p-4 bg-white flex flex-col print:p-3">
            ...
            <div className="mt-auto pt-6 border-t border-slate-100">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="text-[10px] font-bold text-slate-500 uppercase tracking-wider text-center">Customer Name / Signature</div>
                </div>
                <div>
                  <div className="border-b border-slate-400 h-8 mb-1"></div>
                  <div className="text-[10px] font-bold text-slate-500 uppercase tracking-wider text-center">Date</div>
                </div>
              </div>
            </div>
          </div>
...
if (!file.includes('CUSTOMER AUTHORIZATION')) {
  // It replaces at the very end of the main card wrapper
  file = file.replace(/(\s*<\/div>\s*<\/div>\s*)$/, match => '\n' + legalBlocks + match);
}
