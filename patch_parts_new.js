const fs = require('fs')
const path = 'src/app/(dashboard)/parts/new/page.tsx'
...
if (!code.includes('EngineOilClassificationModal')) {
  ...
  // onCategorySelect callback inside handle
  ...
  // PartGroupCategorySelector props
  code = code.replace(
  ...
  `
  // We need to carefully replace the PartGroupCategorySelector without duplicate onCategorySelect
  // Actually, I already added onCategorySelect above. Let's just do a specific replace.
  code = code.replace(
    /<PartGroupCategorySelector[\s\S]*?\/>/,
    uiIndicator.trim()
  )

  // Add the modal at the end before closing form
  code = code.replace(
    /<\/form>/,
  ...
  console.log('Already patched parts/new')
}
