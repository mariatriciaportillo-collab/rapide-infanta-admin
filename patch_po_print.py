...
export default function PurchaseOrderPrintPage({
  ...
  // Group consecutive identical vehicles visually
  let lastVehicleStr = ''
  ...
  return (
    <div className="min-h-screen bg-slate-200 py-8 print:bg-white print:py-0 font-sans text-slate-800">
      ...
      <div className="max-w-5xl mx-auto bg-white shadow-xl print:shadow-none mx-4 md:mx-auto">
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-800 text-white text-xs uppercase tracking-wider">
                  {hasVehicle && <th className="px-4 py-3 font-medium rounded-tl-sm w-48">Vehicle / Unit</th>}
                  {hasVehicle && <th className="px-4 py-3 font-medium w-32">Chassis No.</th>}
                  <th className={`px-4 py-3 font-medium ${!hasVehicle ? 'rounded-tl-sm' : ''}`}>Item / Description</th>
                  <th className="px-4 py-3 font-medium w-24">Part No.</th>
                  <th className="px-4 py-3 font-medium text-right w-16">Qty</th>
                  <th className="px-4 py-3 font-medium text-right w-28">Unit Cost</th>
                  <th className="px-4 py-3 font-medium text-right w-32 rounded-tr-sm">Amount</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-200 border-b-2 border-slate-800">
                {items.length === 0 ? (
                  <tr>
                    <td colSpan={hasVehicle ? 7 : 5} className="px-4 py-6 text-center text-slate-500 italic">No items found</td>
                  </tr>
                ) : (
                  items.map((item: any, i: number) => {
                    ...
                    const vehicleStr = item.manual_vehicle || '—'
                    const showVehicle = hasVehicle && vehicleStr !== lastVehicleStr
                    if (hasVehicle) lastVehicleStr = vehicleStr
                    
                    return (
                      <tr key={i} className="hover:bg-slate-50">
                        {hasVehicle && (
                          <td className="px-4 py-3 text-sm text-slate-800 font-bold border-r border-slate-100">
                            {showVehicle ? vehicleStr : ''}
                          </td>
                        )}
                        {hasVehicle && (
                          <td className="px-4 py-3 text-xs text-slate-500 font-mono border-r border-slate-100">
                            {showVehicle ? (item.chassis_number || '—') : ''}
                          </td>
                        )}
                        <td className="px-4 py-3 text-sm text-slate-800 font-medium">{item.parts?.name || 'Unknown Item'}</td>
                        <td className="px-4 py-3 text-xs text-slate-600 font-mono">{item.parts?.part_number || '—'}</td>
