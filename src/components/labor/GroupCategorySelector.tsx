...
export type LaborGroup = { id: string; name: string }
export type LaborCategory = { id: string; group_id: string; name: string }
...
export function GroupCategorySelector({ 
  ...
  const fetchData = async () => {
    const { data: gData } = await supabase.from('labor_groups').select('*').eq('is_active', true).order('name')
    if (gData) setGroups(gData)

    const { data: cData } = await supabase.from('labor_categories').select('*').eq('is_active', true).order('name')
  ...
  const saveNewGroup = async () => {
    ...
    const { data, error } = await supabase.from('labor_groups').insert({ 
      name: cleanName
    }).select().single()
  ...
  const saveNewCategory = async () => {
    ...
    const { data, error } = await supabase.from('labor_categories').insert({ 
