...
export default function EditPartLaborRulePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      const { data: pData } = await supabase.from('parts').select('id, name, part_number').eq('auto_suggest_labor', true).order('name')
      if (pData) setAvailableParts(pData)

      // Load labor from labor_services
      const { data: lData } = await supabase.from('labor_services').select('id, name').order('name')
      if (lData) setAvailableLabor(lData)

      // Load rule
      const { data: ruleData } = await supabase.from('part_labor_rules').select('*').eq('id', ruleId).single()
      if (ruleData) {
        setRuleName(ruleData.rule_name)
        setRuleType(ruleData.rule_type)
        setLaborId(ruleData.labor_id)
        setActive(ruleData.active)
        setNotes(ruleData.notes || '')
      }

      // Load triggers
      const { data: triggerData } = await supabase.from('part_labor_rule_triggers')
        .select('part_id, parts(id, name, part_number)')
        .eq('rule_id', ruleId)
      
      if (triggerData) {
        const parts = triggerData.map(t => t.parts).filter(Boolean)
        setSelectedParts(parts as any[])
      }
      setLoading(false)
    }
    loadData()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error: rErr } = await supabase.from('part_labor_rules').update({
        rule_name: ruleName,
        rule_type: ruleType,
        labor_id: laborId,
        active,
      ...
      // Recreate triggers
      await supabase.from('part_labor_rule_triggers').delete().eq('rule_id', ruleId)
      const triggers = selectedParts.map(p => ({ rule_id: ruleId, part_id: p.id }))
  ...
  if (loading) return <div className="p-8 text-center text-slate-500">Loading rule...</div>

  return (
    <div className="p-6 max-w-4xl mx-auto">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-slate-800">Edit Part-to-Labor Rule</h1>
      </div>
        ...
        <div className="bg-white border border-slate-200 rounded-lg shadow-sm overflow-visible mb-6">
          ...
          <div className="p-6">
            ...
            {selectedParts.length > 0 ? (
              <div className="flex flex-wrap gap-2">
                {selectedParts.map(p => (
                  <div key={p.id} className="flex items-center gap-2 bg-blue-50 border border-blue-100 text-blue-800 px-3 py-1.5 rounded-full text-sm font-medium">
                    {p.name}
                    <button type="button" onClick={() => handleRemovePart(p.id)} className="text-slate-400 hover:text-red-600">
                      <X size={14} />
                    </button>
                  </div>
                ))}
              ...
              <p className="text-sm text-slate-500 italic">No parts added.</p>
            )}
          </div>
        </div>
