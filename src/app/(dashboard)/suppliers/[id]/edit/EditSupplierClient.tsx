...
export function EditSupplierClient({ id }: { id: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (updateError) {
      setError(`Failed to update supplier: ${updateError.message}`)
      setIsSubmitting(false)
      return
    }
  ...
  if (isLoading) {
    return (
      <div className="max-w-4xl mx-auto py-24 text-center text-slate-500">
        <div className="flex justify-center mb-4"><Truck className="animate-pulse text-slate-300" size={48} /></div>
        <p className="text-xl">Loading supplier details...</p>
      </div>
    )
  }
    ...
    <div className="max-w-4xl mx-auto pb-24">
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-4">
          ...
          <h2 className="text-3xl font-bold text-slate-800">Edit Supplier</h2>
        </div>
      </div>
