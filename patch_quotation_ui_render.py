...
# We need to replace from {/* LINE ITEMS */} to {/* Footer / Totals */}
...
replacement = """        {/* SECTION 1: PACKAGES */}
        ...
        <div className="bg-white border border-slate-200 rounded-lg shadow-sm p-6 mb-6">
          ...
          <div className="space-y-2">
            {items.filter(i => i.item_type === 'MANUAL' || i.item_type === 'LABOR').map((item, index) => (
            ...
            {items.filter(i => i.item_type === 'MANUAL' || i.item_type === 'LABOR').length === 0 && (
        ...
        <div className="bg-white border border-slate-200 rounded-lg shadow-sm p-6 mb-6">
          ...
          <div className="space-y-2">
              ...
              <div key={item.id} className="flex gap-3 items-center py-1">
                <div className="flex-1">
                  <input
                    ...
                    className="w-full border border-slate-300 rounded-md p-2 bg-blue-50 font-medium text-blue-900"
                  />
                </div>
            ...
            {items.filter(i => i.item_type === 'PART').length === 0 && (
              <p className="text-center text-slate-400 py-4">No parts added.</p>
            )}
          </div>
        </div>
...
# We need to find exactly where to replace.
# Looking at current QuotationForm.tsx:
#        {/* ITEM ROWS */}
#        <div className="space-y-2">
...
# It's better to use re.sub with DOTALL
pattern = r"        \{\/\* LINE ITEMS \*\/\}[\s\S]*?\{\/\* Footer \/ Totals \*\/\}|" + r"        \{\/\* ITEM ROWS \*\/\}[\s\S]*?\{\/\* Footer \/ Totals \*\/\}"
# Wait, my previous code replaced the LINE ITEMS block in a messy way because I inserted `grid-cols-2` and `SearchableCombobox` earlier. Let's dump the entire actual content between `<div className="mb-4 relative" ref={searchRef}>` (where customer search ends) and `<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">` (where Footer/Totals starts) to be safe.
